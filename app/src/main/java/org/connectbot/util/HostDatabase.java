...
public class HostDatabase extends RobustSQLiteOpenHelper implements HostStorage, ColorStorage {
	...
	public final static String FIELD_HOST_AUTO_CONNECT = "auto_connect";
	...
	public static final String TABLE_HOSTS_COLUMNS = "_id INTEGER PRIMARY KEY, "
			...
			+ FIELD_HOST_AUTO_CONNECT + " TEXT DEFAULT '" + false + "'";
	...
	public void onRobustUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) throws SQLiteException {
		...
		case 24:
			...
			// fall through
		case 25:
			// Add auto_connect column to hosts table
			db.execSQL("ALTER TABLE " + TABLE_HOSTS
					+ " ADD COLUMN " + FIELD_HOST_AUTO_CONNECT + " TEXT DEFAULT '" + false + "'");
			break;
		}
	}
	...
	private List<HostBean> createHostBeans(Cursor c) {
		...
		final int COL_ID = c.getColumnIndexOrThrow("_id"),
			...
			COL_AUTO_CONNECT = c.getColumnIndexOrThrow(FIELD_HOST_AUTO_CONNECT);
		...
		while (c.moveToNext()) {
			...
			host.setAutoConnect(Boolean.parseBoolean(c.getString(COL_AUTO_CONNECT)));
